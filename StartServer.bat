...
SET BEPATH = A:\SteamLibrary\steamapps\common\Arma 3 Server\battleye
...
start "%START_NAME%+%PORT%" /HIGH %ARMA_EXE% -port=%PORT% -bandwidthAlg=%BANDWIDTH% -limitFPS=%FPS% -config=%SERVER_CFG% -cfg=%BASIC_CFG% -profiles=%PROFILE% -filePatching "-servermod=%Smod%" "-mod=%Mod%" "mods_required\@c1\userconfig\cba_settings.sqf"
timeout %TIME_OUT% /nobreak